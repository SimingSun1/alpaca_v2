...
class ALPaCA:
    ...
    def construct_model(self):
        with self.graph.as_default():
            ...
            self.K = tf.compat.v1.get_variable('K_init',shape=[last_layer,self.y_dim]) #\bar{K}_0

            self.L_asym = tf.compat.v1.get_variable('L_asym',shape=[last_layer,last_layer]) # cholesky decomp of \Lambda_0
            ...
            self.x = tf.compat.v1.placeholder(tf.float32, shape=[None,None,self.x_dim], name="x")
            self.y = tf.compat.v1.placeholder(tf.float32, shape=[None,None,self.y_dim], name="y")
            ...
            self.context_x = tf.compat.v1.placeholder(tf.float32, shape=[None,None,self.x_dim], name="cx")
            self.context_y = tf.compat.v1.placeholder(tf.float32, shape=[None,None,self.y_dim], name="cy")
            ...
            self.num_context = tf.compat.v1.placeholder_with_default(self.max_num_context, shape=(None,))
            ...
            with tf.compat.v1.variable_scope('blr', reuse=None):
            ...
            self.train_writer = tf.compat.v1.summary.FileWriter('summaries/'+str(time.time()), self.sess.graph, flush_secs=10)
    ...
    def basis(self,x,name='basis'):
        ...
        with tf.compat.v1.variable_scope(name,reuse=tf.compat.v1.AUTO_REUSE):
            for units in layer_sizes:
                inp = tf.compat.v1.layers.dense(inputs=inp, units=units,activation=activation)
    ...
    def batch_blr(self,X,Y,num):
        X = X[:num,:]
        Y = Y[:num,:]
        Ln_inv = tf.linalg.inv(tf.transpose(X) @ X + self.L)
    ...
    def compute_pred_and_nll(self):
        ...
        #mu_pred = batch_matmul(tf.transpose(self.posterior_K), self.phi) + self.f_nom_x
        mu_pred = batch_matmul(tf.linalg.matrix_transpose(self.posterior_K), self.phi) + self.f_nom_x
        ...
        with tf.compat.v1.variable_scope('loss', reuse=None):
            logdet = self.y_dim*tf.math.log(spread_fac) + tf.linalg.logdet(self.SigEps)
...
def batch_quadform(A, b):
    ...
    if A_dims == b_dims + 1:
        ...
        Ab = tf.expand_dims( tf.linalg.matrix_transpose( A @ tf.linalg.matrix_transpose(b) ), axis=-1) # ... x N x n x 1

        return tf.squeeze( tf.linalg.matrix_transpose(b_vec) @ Ab, axis = -1) # ... x N x 1
    else:
